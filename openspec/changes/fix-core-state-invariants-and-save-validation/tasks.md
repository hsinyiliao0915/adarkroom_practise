...
- [ ] 2.2 實作多槽位索引（Save Index）與舊存檔自動相容遷移機制
- [ ] 2.3 實作「建立命名存檔」、「覆蓋現有存檔」、「載入指定存檔」與「刪除存檔」方法，並編寫單元測試驗證
- [ ] 2.4 實作「開啟新遊戲」流程，在保留既有存檔前提下重置活躍狀態為全新開局
- [ ] 2.5 建立 SaveLoadModal 存檔管理彈窗 UI，支援輸入自訂名稱存檔、檢視存檔列表與新遊戲操作
...
- [ ] 5.1 執行 npm test 確保所有存檔槽位、狀態不變量與守衛單元測試通過
